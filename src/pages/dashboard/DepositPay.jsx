...
const DepositPay = () => {
    ...
    //   wallets?.map((props)=> {walletInfo.wallet === props.walletName ? setH(props?.walletName) : "Loading...";})
      ...
      const handlegetallWalletAddress = async () => {
        await axios.get('https://n1-finance-back-end.vercel.app/api/getallWalletAddress')
            .then(response => {
    ...
    const url = `https://n1-finance-back-end.vercel.app/api/sendpayment/${id}`
      const url2 = `https://n1-finance-back-end.vercel.app/api/deposit/${id}
`
      const data = {
